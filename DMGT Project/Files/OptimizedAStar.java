...
public class OptimizedAStar {
    ...
    public static void main(String[] args) {
        OptimizedAStar m = new OptimizedAStar();
        int noOfStations = 380;
        ...
        printAllPaths(graph, visited, src, "", tar, Map, 0.0f, src);

        List<Map.Entry<Float, String>> entryList = new ArrayList<>(Map.entrySet());

        entryList.sort(Comparator.comparing((Map.Entry<Float, String> entry) -> entry.getKey()));

        System.out.println("Shortest Path :");
        if (!entryList.isEmpty()) {
            Map.Entry<Float, String> firstEntry = entryList.get(0);
            System.out.println(firstEntry.getValue() + "\nLength: " + Math.abs(firstEntry.getKey()) + " km");
        } else {
