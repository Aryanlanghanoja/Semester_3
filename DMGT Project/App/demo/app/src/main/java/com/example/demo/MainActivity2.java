...
public class MainActivity2 extends AppCompatActivity {
    ...
    public static void printAllPath(ArrayList<Edge>[] graph, boolean[] vis, String curr, String path, String dest1,
                                    Map<Float, String> Map, float length , String source) {
//        if (curr.equals(dest1)) {
//            Map.put(length, path + " -> " + dest1);
//            return;
//        }
            ...
            for (int i = 0; i < graph[currindex].size(); i++) {
                ...
                if (!vis[desindex]) {
                    if (curr.equals(source)) {
                        ...
                        printAllPath(graph, vis, e.Destination,Routeno + " : " +  curr, dest1, Map, newLength, source);
                        length = length - e.distance;
                    } else {
                        float newLength = length + e.distance;
                        printAllPath(graph, vis, e.Destination, path + "\n" + Routeno + " : " +  curr, dest1, Map, newLength, source);
                        length = length - e.distance;
                    }
                }
            }
    ...
    public void openact2(){
        ...
        Intent intent = new Intent(MainActivity2.this,p2.class);
        intent.putExtra("Loc.", loc1);
        intent.putExtra("Dest.", dest1);
        intent.putExtra("Ans.", qq);
...
//        String dest1 = dest1;
        ...
        printAllPath(graph, visit, loc1, "", dest1, Map, 0.0f , loc1);
        ...
        if (!entryList.isEmpty()) {
         ...
         //   System.out.println("Path: " + firstEntry.getValue() + "\nLength: " + firstEntry.getKey() + " km");
           qq = firstEntry.getValue() + "\nLength: " + firstEntry.getKey() + " km";
        } else {
//            System.out.println("No paths available.");
            qq = "No paths available.";
        }
    } 
}
